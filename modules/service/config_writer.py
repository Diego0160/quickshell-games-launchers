...
def main():
    try:
        ...
        print(json.dumps({"ok": False, "error": "tomlkit not installed — run: pip install tomlkit"}), flush=True)
        sys.exit(1)
    ...
    try:
        ...
        path = find_config()
        if not path:
            print(json.dumps({"ok": False, "error": "config.toml not found"}), flush=True)
            sys.exit(1)
